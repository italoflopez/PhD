function out = factor_estimation_ls_full_order(data, est_par, var_par)%factor_estimation_ls_full(data, inclcode, est_par)
...
lsout = factor_estimation_ls_order(est_data, est_par);
lsout.fac = lsout.fac(:, [5:8,1:4]);
...
ismpl = smpl(calvec,nfirst,nlast,nper);%smpl_HO(est_par.smpl_par)
...
for is = 1:n_series;
    ...
    if size(tmp,1) >= ntmin;
       ...
       if r2_mat(is) < 0.9999;
        [arcoef, ser] = uar(u,n_uarlag);    % AR Coeest_par.smpl_parfficients and ser 
       else;
...
varout = varest(lsout.fac,var_par,est_par.smpl_par);
