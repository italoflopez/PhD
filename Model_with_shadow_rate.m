...
data=synchronize(macro_data,diff_shadow_rate,betas);
data=rmmissing(data);
...
regression_factors=synchronize(named_slow_macro_factors,shadow_rate,named_fast_macro_factors);
regression_factors=rmmissing(regression_factors);

full_sample=synchronize(named_slow_macro_factors,shadow_rate,named_fast_macro_factors,slow_moving_variables,fast_moving_variables);
...
%Now let's see effect of QE on long-term yields
c = fred
fed_balance_sheet = fetch(c,'WALCL');
close(c)
fed_balance_sheet_series = fed_balance_sheet.Data(:,2);
timeseries(fed_balance_sheet_series,fed_balance_sheet.DateRange);