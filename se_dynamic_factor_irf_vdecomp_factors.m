function out = se_dynamic_factor_irf_vdecomp_factors(datain,fac_est_out,est_par,decomp_par,n_rep,var_par)
...
% fac_est_out.varout.coef.Q = fac_est_out.varout.coef.Q([5:8,1:4],: );
  ...
  btstrp_fac_est_out.varout.coef.G = [chol(cov(btstrp_fac_est_out.varout.resid(9:end,:))).';btstrp_fac_est_out.varout.coef.G(9:(lag_order*n_factors),:)];%[chol(cov(btstrp_fac_est_out.varout.resid(9:end,:))).';btstrp_fac_est_out.varout.coef.G(9:24,:)];
...
%   btstrp_fac_est_out.varout.resid = btstrp_fac_est_out.varout.resid(:, [5:6,1,7:8,2:4]);
% % fac_est_out.varout.coef.Q = fac_est_out.varout.coef.Q([5:8,1:4],: );
%   btstrp_fac_est_out.varout.coef.M(1:8,:) = btstrp_fac_est_out.varout.coef.M(1:8,[5:6,1,7:8,2:4,13,14,9,15,16,10,11,12,21,22,17,23,24,18,19,20]);%fac_est_out.varout.coef.M(1:8,[5:6,1,7:8,2:4,13,14,9,15,16,10,11,12,21,22,17,23,24,18,19,20,29,30,25,31,32,26,27,28,37,38,33,39,40,34,35,36,45,46,41,47,48,42,43,44,53,54,49,55,56,50,51,52,61,62,57,63,64,58,59,60])
%   btstrp_fac_est_out.varout.coef.M= btstrp_fac_est_out.varout.coef.M([5:6,1,7:8,2:4,9:24],:);
%   btstrp_fac_est_out.varout.coef.G = [chol(cov(btstrp_fac_est_out.varout.resid(9:end,:))).';btstrp_fac_est_out.varout.coef.G(9:24,:)];
% 
%     bs_est_par = est_par;
%   bs_est_par.fac_par.lambda_constraints_est = bs_est_par.fac_par.lambda_constraints_est(:, [6:7,2,8:9,3:5]);
%   bs_est_par.fac_par.lambda_constraints_full = bs_est_par.fac_par.lambda_constraints_full(:, [6:7,2,8:9,3:5]);
...
%   bs_est_par.fac_par.lambda_constraints_full = bs_est_par.fac_par.lambda_constraints_full(:, [6:9,2,10:11,3:5]);
  ...
  btstrp_irf_vdecomp_out = dynamic_factor_irf_vdecomp_factors(btstrp_fac_est_out,bs_est_par,decomp_par,datain.bptcodevec);   % irf variance decomposition
