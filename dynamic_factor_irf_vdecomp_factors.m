function out = dynamic_factor_irf_vdecomp_factors(fac_est_out, est_par, decomp_par, bptcodevec)
...
size_panel_variables = size(est_data,2);
n_factors = size(fac_est,2);
matrix = zeros(size_panel_variables,n_factors);

% Set the upper-left 8x8 block to be an identity matrix
matrix(1:n_factors, 1:n_factors) = eye(n_factors);
lam_mat       = matrix;
